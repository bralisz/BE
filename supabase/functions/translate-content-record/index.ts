...
const MUSIC_SECTION_IDS = new Set(["14386598-4978-403a-8548-db0ee582e291","18db9515-179c-4bad-9646-1fcda63df14a","e995b960-503c-4d67-8d7c-87cbd6eda6a2"]);
const MUSIC_SECTION_NAMES = new Set(["live performances & tv","videoclipes","concert","concierto"]);
const MUSIC_TITLE_PATTERN = /(^|[\s._/?:=&-])(music|musica|música|song|faixa|track|album|álbum|videoclipe|live performances|concert|concierto)(?=$|[\s._/?:=&-])/i;
  ...
  try{
    for(const row of rows as any[]){
      ...
      for(const locale of locales){
        ...
        const existing=translations[locale];if(!force&&existing&&existing.sourceUpdatedAt===signature){const cached={...existing};if(keepTitle){if(typeof data.title==="string"&&text(data.title))cached.title=data.title;if(typeof data.name==="string"&&text(data.name))cached.name=data.name;}translations[locale]=cached;responseRecords.push({id:row.id,locale,translation:cached,cached:true});continue;}
