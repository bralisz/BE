...
function preserveTitle(collection:string,data:Record<string,unknown>){if(collection==="albums"||collection==="shows")return true;if(collection!=="videos")return false;const sectionName=normalizeTitleContext(data.sectionName||data.sourceSectionTitle||"");const protectedSection=/^(live performances?( and | )tv|videoclipes?|video clips?|music videos?|videos musicales|clips? musicaux|concerts?|concertos?|conciertos?|shows?|festivals?|festivais?)$/.test(sectionName);return MUSIC_SECTION_IDS.has(text(data.sectionId,120))||protectedSection||looksLikeMusicPerformanceTitle(data);}
