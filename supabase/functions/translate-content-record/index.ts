...
const MUSIC_CONTEXT = /(^|\s)(music|musica|song|faixa|track|album|videoclipe|live performances)(\s|$)/;
...
function normalized(value:unknown){return text(value,240).normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase();}
function musicTitle(data:Record<string,unknown>){const context=[data.sectionName,data.sourceSectionTitle,data.category,data.type,data.contentType].map(normalized).filter(Boolean).join(" ");return MUSIC_SECTION_IDS.has(text(data.sectionId,120))||MUSIC_SECTION_NAMES.has(normalized(data.sectionName||data.sourceSectionTitle))||MUSIC_CONTEXT.test(context);}
function preserveTitle(collection:string,data:Record<string,unknown>,locale:string){if(collection==="ongs")return true;if(collection==="videos"&&musicTitle(data))return true;if(locale!=="es")return false;if(collection==="movies"||collection==="news")return true;if(collection==="sections"&&normalized(data.title||data.name)==="vanity fair")return true;return false;}
  ...
  try{
    for(const row of rows as any[]){
      ...
      for(const locale of locales){
        ...
        const existing=translations[locale];if(!force&&existing&&existing.sourceUpdatedAt===signature){
          if(keepTitle&&existing&&typeof existing==="object"){delete existing.title;delete existing.name;}
          responseRecords.push({id:row.id,locale,translation:existing,cached:true});continue;
        }
