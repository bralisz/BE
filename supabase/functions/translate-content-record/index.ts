...
function looksLikeMusicPerformanceTitle(data:Record<string,unknown>){const title=normalizeTitleContext(data.title||data.name||"");if(!title)return false;return /\b(live from|live at|live on|live session|live performance|ao vivo em|ao vivo no|ao vivo na|en vivo desde|en vivo en|performance at|performed at)\b/.test(title)||/\b(official music video|official video|music video|lyric video|official audio|audio oficial|visualizer|visualiser)\b/.test(title);}
function preserveTitle(collection:string,data:Record<string,unknown>){if(data.preserveTitle===true||text(data.preserveTitle,10).toLowerCase()==="true")return true;if(collection==="albums"||collection==="shows")return true;const context=normalizeTitleContext([data.sectionName,data.sourceSectionTitle,data.category,data.type,data.contentType].filter(Boolean).join(" "));if(collection==="news")return Array.isArray(data.tracks)||/\b(album|single|ep|disco|albumes|albums|musica|music)\b/.test(context);if(collection!=="videos")return false;if(MUSIC_SECTION_IDS.has(text(data.sectionId,120)))return true;return /\b(videoclipes?|video clips?|music videos?|videos musicales|clips? musicaux?|musica|music|song|songs|faixa|faixas|tracks?)\b/.test(context)||/\b(live performances?|performances? ao vivo|presentaciones? en vivo|performances? live|concerts?|concertos?|conciertos?|shows?|festivals?|festivais?)\b/.test(context)||looksLikeMusicPerformanceTitle(data);}
  ...
  try{
    for(const row of rows as any[]){
      ...
      for(const locale of locales){
        const existing=translations[locale];const missingRequiredTitle=!keepTitle&&["title","name"].some(field=>typeof data[field]==="string"&&text(data[field])&&(!existing||typeof existing[field]!=="string"||!text(existing[field])));if(!force&&existing&&existing.sourceUpdatedAt===signature&&!missingRequiredTitle){const cached={...existing};if(keepTitle){delete cached.title;delete cached.name;translations[locale]=cached;}responseRecords.push({id:row.id,locale,translation:cached,cached:true});continue;}
