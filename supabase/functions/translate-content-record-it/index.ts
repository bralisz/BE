...
const TARGETS: Record<string,string> = {it:"it"};
...
function allowedOrigin(origin:string){
  ...
  if(origin===SITE_ORIGIN||origin==="https://www.billieilishtv.site"||origin==="https://be-bralis.vercel.app")return true;
...
function preserveTitle(collection:string,data:Record<string,unknown>,locale:string){
  ...
  if(!["es","fr","it"].includes(locale))return false;
...
function translationComplete(data:Record<string,unknown>,translation:Record<string,unknown>,keepTitle:boolean){
  for(const field of FIELDS){
    if(keepTitle&&(field==="title"||field==="name"))continue;
    const source=data[field];
    if(typeof source!=="string"||!text(source)||/^https?:\/\//i.test(text(source)))continue;
    if(typeof translation[field]!=="string"||!text(translation[field]))return false;
  }
  return true;
}
