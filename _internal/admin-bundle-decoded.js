...
(() => {
  ...
  async function usersPage() {
    ...
    let items = (await db.list('users', { orderBy: 'createdAt', direction: 'desc' })).filter(item => !String(item.email || '').toLowerCase().endsWith('@deleted.invalid')).map(item => ({ ...item, banned:userProfileIsBanned(item) }));
  ...
  async function billieSettingsPage() {
    ...
    content.innerHTML = `<div class="admin-title-row"><div><span class="dashboard-kicker">Página especial</span><h1>Conheça a Billie Eilish</h1><p>Edite as imagens, a sincronização e as redes sociais da página.</p></div><a class="a-btn" href="/billie-eilish" target="_blank" rel="noopener noreferrer">Abrir página</a></div><section class="billie-admin-layout"><article class="a-card billie-admin-form-card"><form id="billieSettingsForm" class="billie-admin-form"><div class="billie-admin-source"><label class="field"><span>Fonte das informações</span><select class="a-select" name="sourceMode"><option value="manual" ${settings.sourceMode !== 'wikipedia' ? 'selected' : ''}>Texto salvo anteriormente</option><option value="wikipedia" ${settings.sourceMode === 'wikipedia' ? 'selected' : ''}>Atualização pela Wikipédia</option></select><small>Use a atualização pela Wikipédia para manter as informações da página sincronizadas.</small></label><button class="a-btn" type="button" id="billieWikipediaTest">Buscar na Wikipédia agora</button></div><div class="billie-admin-sync-result" id="billieWikipediaResult" hidden></div><div class="form-grid"><label class="field full"><span>URL do banner compacto da Home</span><input class="a-input" name="bannerUrl" value="${esc(settings.bannerUrl || '')}" placeholder="https://... ou /assets/..."><small>Deixe em branco para manter o banner padrão. Use uma imagem horizontal publicada em /assets ou um link HTTPS de um host aceito pelo proxy, como Imgur, Discord CDN ou Wikimedia.</small></label><label class="field full"><span>URL da foto</span><input class="a-input" name="portraitUrl" value="${esc(settings.portraitUrl || '')}" placeholder="Deixe em branco para usar a foto padrão"><small>Você pode usar um link HTTPS. O campo vazio mantém a foto padrão do site.</small></label><label class="billie-admin-switch field full"><span><strong>Incluir Referências, Ver também e Ligações externas</strong><small>Desativado deixa a página mais limpa; as demais seções do artigo continuam disponíveis.</small></span><input type="checkbox" name="includeReferences" ${settings.includeReferences === true || String(settings.includeReferences) === 'true' ? 'checked' : ''}></label></div><div class="billie-admin-social-heading"><strong>Redes oficiais</strong><small>Os ícones somem da página quando o campo correspondente fica vazio.</small></div><div class="form-grid"><label class="field"><span>Instagram</span><input class="a-input" type="url" name="instagram" value="${esc(settings.instagram)}"></label><label class="field"><span>X / Twitter</span><input class="a-input" type="url" name="xUrl" value="${esc(settings.xUrl)}"></label><label class="field"><span>YouTube</span><input class="a-input" type="url" name="youtube" value="${esc(settings.youtube)}"></label><label class="field"><span>Spotify</span><input class="a-input" type="url" name="spotify" value="${esc(settings.spotify)}"></label><label class="field full"><span>Site oficial</span><input class="a-input" type="url" name="website" value="${esc(settings.website)}"></label></div><div class="modal-actions"><button class="a-btn primary" id="billieSettingsSave" type="submit">Salvar página</button></div></form></article><aside class="a-card billie-admin-preview-card"><span class="dashboard-kicker">Prévia rápida</span><div class="billie-admin-preview-banner">${settings.bannerUrl ? `<img src="${esc(media(settings.bannerUrl))}" alt="">` : '<span>Banner padrão da Home</span>'}</div><div class="billie-admin-preview-image">${settings.portraitUrl ? `<img src="${esc(media(settings.portraitUrl))}" alt="">` : '<span>Foto padrão do site</span>'}</div><strong>Billie Eilish</strong><small>${settings.sourceMode === 'wikipedia' ? 'As informações serão atualizadas pela Wikipédia.' : 'A página usará o texto salvo anteriormente.'}</small></aside></section>`;
