  ...
  async function openEditor(name, item = null) {
    ...
    $('#editorForm').onsubmit = async event => {
      ...
      try {
        ...
        if (typeof data.category === 'string') data.category = data.category.trim().toLowerCase().replace(/\s+/g, '-');
        if (name === 'videos') {
