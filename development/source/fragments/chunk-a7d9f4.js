...
.content-live-preview{position:fixed;top:max(18px,env(safe-area-inset-top));right:max(18px,env(safe-area-inset-right));bottom:max(18px,env(safe-area-inset-bottom));z-index:142;width:min(660px,calc(100vw - 36px));min-height:0;max-height:none;overflow:auto;padding:22px;border:1px solid rgba(125,181,255,.22);border-radius:28px;background:radial-gradient(circle at 80% 10%,rgba(56,132,244,.12),transparent 34%),#080b11;box-shadow:0 34px 100px rgba(0,0,0,.62);opacity:0;visibility:hidden;pointer-events:none;transform:translateX(calc(100% + 42px)) scale(.985);transition:transform .26s cubic-bezier(.2,.8,.2,1),opacity .2s ease,visibility .2s ease;overscroll-behavior:contain}
...
(() => {
  ...
  function setupModernContentEditor(root, name, item, draftKey, restoredDraft) {
    ...
    const updatePreview = () => {
      ...
      previewDescription.innerHTML = adminMarkdownToHtml(descriptionText);
