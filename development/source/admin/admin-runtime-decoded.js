...
(() => {
  ...
  async function dashboard() {
    ...
    const loadDeploymentVersion = async () => {
      const response = await fetch('/api/deployment-version?versionOnly=1', {
        method: 'GET',
        cache: 'no-store',
    ...
    const deploymentShortLabel = deploymentVersion && !deploymentVersion.startsWith('local:')
    ...
    if (dashboardReleaseUpdate) {
      dashboardReleaseUpdate.addEventListener('change', async () => {
        const enabled = dashboardReleaseUpdate.checked;
        const panel = dashboardReleaseUpdate.closest('.dashboard-update-notice');
        const copy = panel && panel.querySelector('.dashboard-update-checkbox-copy small');

        if (enabled && !deploymentCanBeReleased) {
          dashboardReleaseUpdate.checked = false;
          toast('Não foi possível identificar a versão atual do site.', 'err');
          return;
        }
        ...
        try {
          const releaseData = {
            updateReleaseEnabled: enabled,
            releasedDeploymentVersion: enabled ? deploymentVersion : releasedDeploymentVersion,
          ...
          await logAction(
            enabled ? 'site_update_notification_enabled' : 'site_update_notification_disabled',
            'settings',
            'site',
            enabled ? `Aviso da atualização ${deploymentShortLabel} ativado` : 'Aviso de atualização desativado'
          );
          if (panel) panel.classList.toggle('is-released', enabled);
          if (copy) copy.textContent = enabled
            ? 'Ativado. Os usuários verão o aviso desta versão uma única vez.'
            : 'Desativado. Ative apenas quando quiser avisar sobre uma atualização importante.';
          toast(enabled ? 'Notificação de atualização ativada.' : 'Notificação de atualização desativada.');
        } catch (error) {
          dashboardReleaseUpdate.checked = !enabled;
          if (panel) panel.classList.toggle('is-released', !enabled);
          toast(error.message || 'Não foi possível alterar a notificação de atualização.', 'err');
        } finally {
...
(()=>{
  ...
  style.textContent=`
    ...
    body.admin-mode .dashboard-update-notice{
      display:flex!important;
      align-items:center!important;
    ...
    body.admin-mode .dashboard-update-notice.is-released{
